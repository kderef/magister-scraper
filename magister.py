...
if platform.system() == "Windows":
    GECKODRIVER = join(getcwd(), "geckodriver.exe")
else:
    GECKODRIVER = join(getcwd(), "geckodriver")
...
class Cijfer:
    ...
    @property
    def all(self):
        return [self.vak, self.description, self.cijfer, self.weging, self.date, self.inhalen]
...
class Magister:
    def __init__(self, school: str, login_data: tuple, nobrowser=True) -> None:
        ...
        * nobrowser (bool) -> if True, make browser window invisible.
        ...
        log("INFO", '[driver path] = "{}"'.format(GECKODRIVER))
        firefox_opts = FirefoxOptions()
        firefox_opts.headless = nobrowser

        log("INFO", "starting client...")
        self.driver = Firefox(options=firefox_opts, executable_path=GECKODRIVER)
    ...
    def leermiddelen(self) -> dict:
        ...
        for second in range(1, 6):
            print("\033[92mINFO :\033[0m waiting for leermiddelen to load... [{}/5]".format(second), end='\r')
            sleep(1)
        print()
        ...
        leermiddelen = [
            list(y) for x, y in itertools.groupby(result, lambda z: z == "Digitaal") if not x
        ]
            ...
            leermiddelen_dict.append(
                {
                    'vak': item[0],
                    'titel': item[1],
                    'url': self.driver.find_elements_by_tag_name("a")(item[1]).get_attribute('href'),
                    'ean': item[2]
                }
            )
    ...
    def cijfers(self, float_notation=',') -> list[Cijfer]:
        ...
        for i in cijfers_spl:
            c = Cijfer(i[0], i[1], i[2], i[3].replace(",", "."), i[4], i[3] == "Inh") if float_notation != ',' else \
                Cijfer(i[0], i[1], i[2], i[3], i[4], i[3] == "Inh")
            cijfers.append(c)
    ...
    def cijfers_all(self):
        ...
        while True:
            try:
                btn = self.driver.find_element_by_tag_name("dna-button") # dna-button = 'uitgebreide weergave'
            except NoSuchElementException:
                pass
            else:
                btn.click()
